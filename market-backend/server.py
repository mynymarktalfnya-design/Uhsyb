...
@app.middleware("http")
async def persist_local_changes(request: Request, call_next):
    """Keep local/offline data durable after every successful mutation."""
...
def health():
    try:
        db.command("ping")
        return {
            "status": "ok" if (not USING_MOCK_MONGO or USING_LOCAL_PERSISTENCE) else "degraded",
            "db": DB_BACKEND,
            "persistent": not USING_MOCK_MONGO or USING_LOCAL_PERSISTENCE,
            "local_mode": USING_LOCAL_PERSISTENCE,
        }
...
def on_startup():
    try:
        ...
        logger.warning(f"local database restore skipped: {e}")
    try:
        init_indexes()
    except Exception as e:
